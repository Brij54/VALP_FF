...
const { data: metaData, isLoading, error } = useQuery({
  ...
  <div>
  <div>
    <div id="id-6L" className="d-flex flex-column border border-2 p-2 gap-2 mb-2"><div className="border-0 fw-bold fs-3" id="id-6N">Dean</div><div className="border-0 fw-bold" id="id-6R">name *</div><input type="text" className="form-control" name="name" required={true} value={dataToSave["name"] || ""} onChange={(e) => setDataToSave({ ...dataToSave, ["name"]: e.target.value }) }
/><div className="border-0 fw-bold" id="id-6X">signature *</div><div className="mb-3" id="id-6Z"><label className="form-label">Upload file for signature </label><input className="form-control" type="file" name="signature" required={true} onChange={(e) => setDataToSave({ ...dataToSave, ["signature"]: e.target.files?.[0] || null }) }
/></div><div className="border-0 fw-bold" id="id-73">updated_at</div><input type="date" className="form-control" name="updated_at" required={false} value={dataToSave["updated_at"] || ""} onChange={(e) => setDataToSave({ ...dataToSave, ["updated_at"]: e.target.value }) }
/><button className="btn btn-success" id="id-77" onClick={handleCreate}>Submit</button></div>
    {showToast && (
      <div
