...
const UpdateDean = () => {
  ...
  useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await authFetch(`${apiUrl}${params.toString()}`, {
        method: "GET",
      });

      if (!response.ok) throw new Error("Error loading dean data");
  ...
  useQuery({
    ...
    queryFn: async () => {
      ...
      if (!response.ok) throw new Error("Error loading metadata");
    ...
    const columns: ColDef[] = fields.map((field) => {
      if (field === "signature") {
        return {
          ...
          width: 160,
          cellRenderer: (params: any) => (
            <ViewButton documentId={params.value} onView={handleViewSignature} />
          ),
        };
      }
  ...
  const handleViewSignature = async (documentId: any) => {
    try {
      const url =
        `${apiConfig.API_BASE_URL}/dean` +
        `?document_id=${documentId}&queryId=GET_DOCUMENT&dmsRole=admin&user_id=admin@rasp.com`;
  ...
  // ---------------- EDIT NAVIGATION ----------------
  const handleUpdate = (id: any) => navigate(`/edit/dean/${id}`);
